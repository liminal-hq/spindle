...
- `hold` steps: the segment keeps the earlier keyframe's value with no interpolation. The DCSQ lowering is stepped sampling over the _global_ timestamp union: `build_overlay_keyframe_schedule` evaluates every track at the union of every relevant track's keyframe timestamps and holds the result until the next boundary. At a track's _own_ keyframes every easing yields the keyframe's exact value, but at a timestamp another track contributed, an eased track is evaluated mid-segment and its _interpolated_ value is baked into that overlay frame — only all-`hold` tracks are exact-by-construction at every boundary.
...
- **Bed windows are continuous across intro+loop — for an authored bed**: with an `audioAssetId`, the intro plays bed `[0..introDur)` and the loop plays `[introDur..introDur+loopDur)`, so first-play audio doesn't hiccup at the intro/loop boundary. (On the second and later loop passes the bed restarts at its loop-window offset — an accepted artefact of cell-based looping.) Fallback (2) instead trims the background video's own audio to each segment's _source-relative_ window, so its continuity across the first intro-to-loop transition holds only when the intro window ends where the loop window starts (`introStartSecs + introDurationSecs == loopStartSecs`); non-contiguous windows will jump in audio there, matching their jump in video.
...
- Per-frame overlay image paths are `{base}_hl_k{i}.png` / `{base}_sel_k{i}.png`; the executor renders one highlight/select PNG pair per frame through the existing Skia overlay renderer (`generate_menu_overlay_images_for_keyframes`). Anti-aliasing stays off for every frame on the normal path — spumux's ≤16-colour palette limit applies to each one. The exception is the `quantize_overlay_palette` developer option: `render_menu_overlay_image_skia_quantized` deliberately renders WITH anti-aliasing and then reduces the result to a 4-entry palette, so AA'd and normal overlays are not byte-comparable across that flag.
...
- **Canvas video preview**: a motion menu's background renders as a real `<video>` element (`convertFileSrc` over the asset's source path), seeked to the loop start in design mode. Playback state lives in a dedicated zustand store (`store/menu-playback-store.ts`), outside the project store so scrubbing never enters undo history; the playhead is driven by a rAF loop (`useVideoPlayhead`), not the ~4 Hz `timeupdate` event. On Linux, WebKitGTK cannot stream media over the custom `asset://` scheme (plain fetches through it work fine), so after the element's load fails past its scope-grant retry the preview falls back to fetching the file through the asset protocol — same scope enforcement — and playing it from an in-memory blob URL (capped at 512 MiB; past the cap the placeholder shows). The disc build never touches this path.
...
- **Preview animates**: the navigation preview samples colours from the menu's animation tracks — `highlight-colour`/`highlight-opacity` for the focused state, `activate-colour`/`activate-opacity` for the activated state — with opacity baked into the outline's alpha for BOTH states, mirroring the disc's alpha-baked `highlight_colour`/`select_colour` handling. Falls back to the menu's static highlight colours when there's no track (`sampleTrackForPreview`/`sampleHonestPreview`/`sampleHonestFold` in `components/menus/timeline/timelineUtils.ts`). Sampling mirrors the disc's own fidelity rules: only tracks targeting a top-level (compiled) button are ever relevant, the same `menu_ref.buttons()` scope the planner restricts to (a track on a group-nested or non-button node, named by `menu.animation-node-not-compiled`, is excluded the same way the planner drops it); a still menu with tracks previews only the track's first keyframe regardless of playhead, matching the disc's still-menu degrade path (`build_overlay_keyframe_schedule`); a motion menu previews each button's own track, continuously eased, by default — a friendlier view than the disc produces — or, with honest preview enabled, folds every button's relevant track into the SAME menu-wide value (document-order last-track-wins, matching the disc's one CLUT) and quantises to the disc's actual DCSQ schedule boundary — the union of every relevant track's keyframe timestamps across BOTH the highlight and activate groups together, each clamped to the same standard-aware last-presentable frame (`loopDuration − 1 frame`, via `fpsForStandard`) the planner uses, matching it exactly.
...
| `menu.animation-node-not-compiled`        | Warning  | A highlight/activate-state animation track targets a node the planner never resolves to a compiled top-level button — either a button nested inside a group (not yet flattened to the disc) or a non-button node (text/image/shape/group) entirely — so the track has no effect on the build.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
| `menu.animation-easing-quantised`         | Warning  | On a DVD project's motion menu, a highlight/activate colour or opacity track uses an easing curve other than "Hold" on a keyframe that begins a segment (any keyframe except the track's last — a final keyframe's easing has no following segment to shape, so it alone never warns). Still menus and `opacity`/`position` tracks never raise this code. The editor preview interpolates smoothly, but DVD-Video's subpicture overlay can only step to a new image at each keyframe, so the disc will cut instantly instead of easing.                                                                                                                                                                                                                                                           |
| `menu.animation-palette-exhausted`        | Warning  | The animated highlight/activate schedule's distinct colours, plus the reserved transparent-background and button-outline-stroke entries, exceed the 16-entry CLUT every subpicture stream in a PGC must share. The estimate counts only the colour values stored literally in keyframes — an eased colour track evaluated at another track's schedule boundary can bake additional interpolated colours the count misses (a false negative), while colours that never coexist can inflate it (a false positive).                                                                                                                                                                                                                                                                                  |
...
| `menu.animation-keyframe-density`         | Warning  | The overlay schedule — unioned across all four properties (highlight-colour/-opacity and activate-colour/-opacity, the same union the DCSQ lowering shares into one schedule) — exceeds ~1 frame/second; each frame is a full re-rendered subpicture image, risking the ~3.36 Mbit/s subpicture budget. Note: this estimate (and the palette one above) counts every non-empty highlight/activate track, including tracks the planner drops for targeting an uncompiled node (`menu.animation-node-not-compiled`), and clamps timestamps to `loopDuration` where the planner clamps to `loopDuration − 1 frame` (a keyframe at the last-presentable frame plus one at the exact loop end count as two frames here but collapse to one on the disc) — both make it overcount relative to the disc. |
...
| `menu.scene-dangling-background`          | Error    | The scene's background references an asset that no longer exists — for a motion menu, this is the background video itself.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| `menu.scene-dangling-video`               | Error    | A video scene node references an asset that no longer exists.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
