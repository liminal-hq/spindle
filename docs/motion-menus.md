...
| `menu.animation-easing-quantised`         | Warning  | A track uses an easing curve other than "Hold" — the editor preview interpolates smoothly, but DVD-Video's subpicture overlay can only step to a new image at each keyframe, so the disc will cut instantly instead of easing.                                                                                                      |
| `menu.animation-palette-exhausted`        | Warning  | The animated highlight/activate schedule's distinct colours, plus the reserved transparent-background and button-outline-stroke entries, exceed the 16-entry CLUT every subpicture stream in a PGC must share. A conservative estimate: counts colours that could appear anywhere in the schedule, not just ones that must coexist. |
