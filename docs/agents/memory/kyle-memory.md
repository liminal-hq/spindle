...
- **Menu Overhaul Risks (2026-04-05)**:
  ...
  - **Set 2b Workspace Upgrades (2026-04-07)**:
    ...
    - **RESOLVED — Stream Index Validation (commit e80ccc2)**: `validate_action()` now validates `SetAudioStream` and `SetSubtitleStream` against `titleset_stream_counts`, which derives `(audio_count, subtitle_count)` using max-across-titles. Global menus skip validation (correct: no titleset context at VMGM level). `_ => {}` replaced by `PlaybackAction::Stop => {}` — exhaustive match. 12 tests cover all boundary conditions. Backend sealed.
