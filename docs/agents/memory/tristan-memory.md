...
- `MenuDocument.compilePolicy.displayAspect` is the authoritative per-menu DVD display-shape field. Frontend aspect inference should exist only as a compatibility fallback for older menu documents, not as a preview-only override state.
