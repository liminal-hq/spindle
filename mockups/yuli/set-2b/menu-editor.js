...
(function () {
	...
	document.querySelectorAll('[data-nav-view]').forEach((btn) => {
		btn.addEventListener('click', () => {
			document
				...
				.forEach((b) => b.classList.remove('nav-view-btn--active'));
	...
	document.querySelectorAll('.menu-item').forEach((item) => {
		item.addEventListener('click', () => {
			...
			document
				...
				.forEach((i) => i.classList.remove('menu-item--selected'));
			...
			document
				...
				.forEach((n) => n.classList.remove('map-node--selected'));
			...
			document
				.querySelectorAll('.map-menu-card')
				.forEach((c) => c.classList.remove('map-menu-card--selected'));
	...
	document.querySelectorAll('.scene-btn').forEach((btn) => {
		...
		btn.addEventListener('click', (e) => {
			...
			document
				...
				.forEach((b) => b.classList.remove('scene-btn--selected'));
	...
	document.getElementById('previewToggle').addEventListener('click', () => {
		...
		document
			...
			.classList.toggle('preview-toggle--active', previewMode);
	...
	document.querySelectorAll('.canvas-tool-btn[data-tool]').forEach((btn) => {
		btn.addEventListener('click', () => {
			document
				.querySelectorAll('.canvas-tool-btn[data-tool]')
				.forEach((b) => b.classList.remove('canvas-tool-btn--active'));
	...
	document.getElementById('toggleSafeBtn').addEventListener('click', () => {
		...
		document
			.getElementById('toggleSafeBtn')
			.classList.toggle('canvas-tool-btn--active', safeAreasVisible);
	});
	...
	document.getElementById('toggleNavArrows').addEventListener('click', () => {
		...
		document
			.getElementById('toggleNavArrows')
			.classList.toggle('canvas-tool-btn--active', navArrowsVisible);
	});
	...
	document.getElementById('templateToggle').addEventListener('click', () => {
		...
		document
			.getElementById('templateChevron')
			.classList.toggle('template-toggle__chevron--open', templateOpen);
	});
	...
	document.querySelectorAll('.state-chip').forEach((chip) => {
		chip.addEventListener('click', () => {
			document
				...
				.forEach((c) => c.classList.remove('state-chip--active'));
	...
	function drawMapConnections() {
		...
		mapConnections.forEach((conn) => {
			...
			if (conn.fromEdge === 'right' && conn.toEdge === 'left') {
				...
				cp1x = from.x - offset;
				cp1y = from.y - offset * 0.5;
				cp2x = to.x + offset;
				cp2y = to.y - offset * 0.3;
	...
	document.querySelectorAll('[data-bg-mode]').forEach((chip) => {
		chip.addEventListener('click', () => {
			document
				.querySelectorAll('[data-bg-mode]')
				.forEach((c) => c.classList.remove('bg-mode-chip--active'));
			chip.classList.add('bg-mode-chip--active');

			const mode = chip.dataset.bgMode;
			document.getElementById('bgModeBadge').textContent =
			...
			if (mode === 'video') {
				...
				canvas.style.boxShadow =
					'0 8px 40px rgba(0,0,0,0.5), inset 0 0 60px rgba(167,139,250,0.04)';
			} else {
				canvas.style.boxShadow = '';
			}
		});
	...
	document.getElementById('expandAllBtn')?.addEventListener('click', () => {
		document
			.querySelectorAll('#inspectorBody .insp-section')
			.forEach((s) => s.classList.add('insp-section--open'));
	});

	document.getElementById('collapseAllBtn')?.addEventListener('click', () => {
		document
			.querySelectorAll('#inspectorBody .insp-section')
			.forEach((s) => s.classList.remove('insp-section--open'));
	});
	...
	document.querySelectorAll('.style-state-tab').forEach((tab) => {
		tab.addEventListener('click', () => {
			document
				.querySelectorAll('.style-state-tab')
				.forEach((t) => t.classList.remove('style-state-tab--active'));
	...
	document.querySelectorAll('.align-btn').forEach((btn) => {
		btn.addEventListener('click', () => {
			document
				...
				.forEach((b) => b.classList.remove('align-btn--active'));
	...
	document.addEventListener('keydown', (e) => {
		...
		switch (e.key.toLowerCase()) {
			...
			case 't':
				...
				break;
			case 'b':
				document.querySelector('[data-tool="button"]')?.click();
				break;
			case 'i':
			...
			case 'r':
				...
				break;
			case 's':
				document.getElementById('toggleSafeBtn')?.click();
				break;
			case 'n':
				document.getElementById('toggleNavArrows')?.click();
				break;
			case 'p':
