...
export function Statusbar() {
  ...
  const titleCount = project.disc.titlesets.reduce(
    (sum, ts) => sum + ts.titles.length,
    0,
  );
  const menuCount =
  ...
  return (
    <footer className="statusbar">
      ...
      <div className="statusbar__segment">
        {CAPACITY_LABELS[project.disc.capacityTarget]}
      </div>
      <div className="statusbar__segment">
        {titleCount} {titleCount === 1 ? "title" : "titles"} &middot;{" "}
