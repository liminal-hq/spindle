      ...
      <div className="card build__controls">
        ...
        <div className="build__summary">
          <div className="build__summary-item">
            ...
            <span>{disc.standard} · {disc.capacityTarget}</span>
          </div>
          <div className="build__summary-item">
        ...
        {errorCount > 0 && (
          <p className="build__warning">
            {errorCount} validation error{errorCount === 1 ? "" : "s"} must be
            resolved before building.
          </p>
        )}
        {titleCount === 0 && (
          <p className="build__warning">
            Add at least one title to the project before building.
          </p>
        )}
      </div>
        ...
        <div className="card build__log">
          <div className="card__header">
            ...
            <button
              ...
              onClick={() => { setBuildLog([]); setBuildStatus("idle"); }}
            >
              Clear
            </button>
          </div>
          <pre className="build__log-output">
            {buildLog.join("\n")}
          </pre>
        </div>
      )}
        ...
        <div className="card build__validation">
          ...
          <div className="build__issue-list">
            {validationIssues.map((issue, i) => (
              <div
                key={i}
                className={`build__issue build__issue--${issue.severity}`}
              >
