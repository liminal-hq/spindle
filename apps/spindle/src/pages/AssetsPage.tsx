  ...
  return (
    <div className="assets__empty">
      ...
      <p className="text-muted">
        Import video, audio, or subtitle files to get started. Spindle will
        inspect each file and check DVD compatibility.
      </p>
      ...
      <div className="assets__row-main">
        ...
        <div className="assets__row-meta text-muted">
          {asset.durationSecs != null && (
            <span>{formatDuration(asset.durationSecs)}</span>
          )}
      ...
      <div className="assets__row-badges">
        {asset.videoStreams.length > 0 && (
          <span className="badge badge--neutral">
            {asset.videoStreams.length} video
          </span>
        )}
        {asset.audioStreams.length > 0 && (
          <span className="badge badge--neutral">
            {asset.audioStreams.length} audio
          </span>
        )}
        {asset.subtitleStreams.length > 0 && (
          <span className="badge badge--neutral">
            {asset.subtitleStreams.length} sub
          </span>
        )}
      ...
      <div className="assets__detail-section">
        ...
        <dl className="assets__detail-grid">
          ...
          {asset.fingerprint && (
            <>
              <dt>Fingerprint</dt>
              <dd className="assets__detail-mono">
        ...
        <div className="assets__detail-section">
          ...
          {asset.videoStreams.map((vs) => (
            <div key={vs.index} className="assets__stream-item">
              ...
              {vs.frameRate && (
                <span className="text-muted">{vs.frameRate.toFixed(2)} fps</span>
              )}
              {vs.aspectRatio && (
                <span className="text-muted">{vs.aspectRatio}</span>
              )}
              {vs.scanType && (
                <span className="text-muted">{vs.scanType}</span>
              )}
            </div>
          ))}
        </div>
        ...
        <div className="assets__detail-section">
          ...
          {asset.audioStreams.map((as_) => (
            <div key={as_.index} className="assets__stream-item">
              ...
              {as_.language && (
                <span className="text-muted">{as_.language}</span>
              )}
              {as_.bitrateBps && (
                <span className="text-muted">
                  {(as_.bitrateBps / 1000).toFixed(0)} kbps
                </span>
              )}
            </div>
          ))}
        ...
        <div className="assets__detail-section">
          ...
          {asset.subtitleStreams.map((ss) => (
            <div key={ss.index} className="assets__stream-item">
              ...
              {ss.language && (
                <span className="text-muted">{ss.language}</span>
              )}
            </div>
          ))}
        </div>
...
function CompatibilityBadge({
  ...
  compat: CompatibilityAssessment | null;
}) {
  ...
  return (
    <span className={`badge ${classMap[compat]}`}>{labelMap[compat]}</span>
  );
}
