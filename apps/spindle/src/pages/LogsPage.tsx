		...
		<div className="logs">
			...
			<div className="card logs__report">
				...
				<dl className="logs__report-grid">
					...
					<dt>Format</dt>
					<dd>
						{disc.family === 'dvd-video' ? 'DVD-Video' : disc.family} · {disc.standard} ·{' '}
						{disc.capacityTarget}
					</dd>
					<dt>Assets</dt>
			...
			<div className="card logs__validation">
				...
				{validationIssues.length === 0 ? (
					<p className="text-muted">No validation issues. Run validation to check for problems.</p>
				) : (
					<div className="logs__issue-list">
						{validationIssues.map((issue, i) => (
							<div key={i} className={`logs__issue logs__issue--${issue.severity}`}>
								<span className="logs__issue-dot" />
								<span
									className={`logs__issue-severity badge badge--${issue.severity === 'error' ? 'unsupported' : issue.severity === 'warning' ? 'reencode' : 'neutral'}`}
								>
									{issue.severity}
								</span>
						...
						{project.assets.map((asset) => (
							<div key={asset.id} className="logs__asset-row">
								...
								<span className="logs__asset-detail text-muted">
									{asset.containerFormat ?? '?'} · {asset.videoStreams.length}v /{' '}
									{asset.audioStreams.length}a / {asset.subtitleStreams.length}s
								</span>
								<span
									className={`badge ${
