...
- **Animated Highlights**: Keyframe tracks on button highlight/activate colour and opacity, lowered to timestamped subpicture schedules on DVD. The disc can only change a subpicture bitmap at a keyframe timestamp and holds each value until the next one, so a `linear`/`ease-in`/`ease-out`/`ease-in-out` track is evaluated identically in the editor and the build only for Hold easing (or in 'Honest Preview' — see below); the normal editor preview interpolates continuously between keyframes as a friendlier authoring view, and authoring a non-hold easing on a highlight/activate track of a DVD motion menu raises the `menu.animation-easing-quantised` warning (including on a track the planner ultimately drops as uncompiled — that track additionally gets `menu.animation-node-not-compiled`).
...
### 14.5 Generated menu navigation

Autogenerated menus (currently chapter-selection grids, with the same generator machinery available to other generated menu types) ship with fully wired remote-navigation, not placeholder buttons the user must connect manually.

- Every generated button gets up/down/left/right links from its layout geometry — a grid wires row/column neighbours, a single-column list wires a simple vertical chain.
- Every link the generator creates is bidirectional: linking a button's `navRight` to a neighbour also sets that neighbour's `navLeft` back to it, and so on for the other three directions. This guarantees no generated button is reachable only in one direction and unreachable in the other, which would strand it behind a remote press with no way back.
- A trailing utility row (Previous/Back/Next, for paginated chapter grids) is chained left-to-right by position and anchored onto the closest column of the grid's last row, so the utility row and the grid form a single connected navigation graph rather than two disjoint ones.
- Each generated menu sets `interaction.defaultFocusId` to a real button (the first grid item, falling back to the first utility button for pages with no grid items) so the menu has a sane initial remote-navigation focus the moment it's generated, before any manual editing.

Generated menus remain fully editable after creation — a user can rewire, add, or remove any of these links from the authored navigation model like any hand-authored menu.

### 14.6 Menu role model and picker

Every menu carries a `role` — Root, Title Select, Chapter, Setup, Extras, or Popup — describing what the menu is _for_, independent of `domain`, the DVD backend's actual VMGM/titleset placement. Backends map role to physical placement; the terminology layer maps role to the on-screen wording for the project's disc format (e.g. DVD's Root role reads "VMGM Title Menu").

Role is inferred, not required to be hand-picked, using this precedence:

1. Generation metadata — a menu built by the chapter-grid generator infers Chapter, audio/subtitle-setup generators infer Setup.
2. Interaction content — majority vote between `playChapter` and `setAudioStream`/`setSubtitleStream` actions across the menu's buttons, with the menu's name as a weak tiebreaker on a tie.
3. VMGM entry-menu position — the disc's first global menu infers Root.
4. Closed-set fallback — Title Select.

This inference runs once per menu, on load, only when the menu's `role` is genuinely absent (an old project file written before the field existed, or a document that hasn't had a role chosen yet). A role explicitly persisted by the user — Title Select included — is never silently revised by a later load, even if the menu's generation metadata or content would infer something more specific.

The role picker (in the menu-level inspector) only offers roles compatible with the menu's current physical placement: VMGM-compatible roles (Root, Title Select, and Popup where the disc format's `FormatProfile` supports it) for a global menu, titleset-compatible roles (Chapter, Setup, Extras) for a titleset menu. Changing a menu's role never moves it between the VMGM and titleset collections — that's a bigger feature than the picker attempts. If a project ever carries a role incompatible with a menu's placement (a hand-edited file, or one saved before this restriction existed), validation flags it as a Warning rather than silently ignoring the role or blocking the build.
...
9. ~~Should DTS output require a separate advanced-compatibility acknowledgement even when toolchain support is present?~~ Narrowed by the confirmed-broken ffmpeg DTS encoder (§12.2.4): DTS re-encoding isn't offered at all, toolchain-independent, so the open question is now only about copy-mode DTS passthrough, which is already unrestricted since it never touches the broken encoder.
