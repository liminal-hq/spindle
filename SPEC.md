...
- multiple menu pages
- titleset editor with compatibility guidance
...
For v1, the application should use a **guided model with optional advanced visibility**.

That means:

- the internal project model always stores titles within titlesets
- the default UI does not force beginners to manually create and manage titlesets first
- the app may automatically group titles into a default titleset during early project setup
- an advanced project-structure view can expose titlesets explicitly for users who need technical control
...
The app should support both mental models without forcing the more complex one first.

### 26.1.3 Recommended UI behaviour

#### Default mode

- Titles appear in a simple ordered list.
- The app assigns them to a default titleset automatically.
- If all titles are compatible enough to remain grouped, the user does not need to think about titlesets.
...
v1 should support titlesets as a real internal concept, but direct titleset editing should be optional and progressively disclosed.
