...
- **Animated Highlights**: Keyframe tracks on button highlight/activate colour and opacity, lowered to timestamped subpicture schedules on DVD. The disc can only change a subpicture bitmap at a keyframe timestamp and holds each value until the next one, so a `linear`/`ease-in`/`ease-out`/`ease-in-out` track is evaluated identically in the editor and the build only for Hold easing (or in 'Honest Preview' — see below); the normal editor preview interpolates continuously between keyframes as a friendlier authoring view, and authoring a non-hold easing on a track that gets DVD-lowered raises the `menu.animation-easing-quantised` warning.
